...
if (!GROQ_API_KEY) {
  console.error("Missing Groq API key. Set GROQ_API_KEY in .env or environment variables.");
  process.exit(1);
}
console.log("Using Groq key (masked):", "****" + GROQ_API_KEY.slice(-5));
...
app.post("/api/ai-parse", async (req, res) => {
  ...
  const apiKey = req.headers['x-api-key'] || GROQ_API_KEY;
...
app.listen(PORT, () => {
  ...
  if (err.code === "EADDRINUSE") {
    console.error(`Port ${PORT} is already in use. Stop the existing service or set a different PORT in .env before restarting.`);
  } else {
